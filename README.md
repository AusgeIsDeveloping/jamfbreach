# jamfbreach
Exploits in the mainly used Jamf System for schools


THESE EXPLOITS ARE ONLY FOR EDUCATIONAL PURPOSES ONLY

IT DOESNT USE ANY REAL EXPLOITS TO CONTROL AS A STUDENT OTHER IPADS

Introducing jamfbreach!

I used that methods to bypass our school restrictions!
I got snitched and my website got snitched. (I guess it's because someone thought it would make him happy)

But there is the chance, that someone i knew snitched my site and wanted to gatekeep my methods.

------------------------------------------------------------------------------

The main issue is, that you can use .mobileconfigs, if you dont have configured your system well.
With only that exploit you can bypass the feature that teacher can watch over you.

THIS ALSO WORKS FOR MULTIPLE DMAs (not only Jamf)

------------------------------------------------------------------------------

You can also bypass the restriction of Jamf to lock your apps.

To do that do following things:


